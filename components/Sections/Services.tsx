...
// Moonstone order pill per category — opens the on-site booking flow.
const btnCard = "inline-flex items-center justify-center gap-1.5 px-6 py-3 rounded-full bg-moon text-plum-deep text-sm font-semibold hover:bg-moon-bright hover:-translate-y-0.5 transition-all duration-300 shadow-[0_0_0_1px_rgba(219,205,242,0.3),0_16px_40px_-20px_rgba(198,178,228,0.7)]";

// A per-category twilight tint — each panel a slightly different note in the same
// indigo/plum family (translucent so the sunrise sky bleeds and shifts through).
const PANELS = ['rgba(23,17,52,0.58)', 'rgba(32,20,60,0.58)', 'rgba(20,23,64,0.58)', 'rgba(30,20,56,0.58)'];
...
const Services: React.FC<ServicesProps> = ({ isIndonesian = false }) => {
    ...
    const groups = isIndonesian
        ? [
            ...
            },
            {
                ...
                offers: [
                    ...
                    },
                    {
                        ...
                        badge: 'Promo', badgeTone: 'bg-moon/20 text-moon-bright',
            ...
            },
            {
                ...
                blurb: <>Temu langsung di Jakarta Selatan — energi lebih terasa, analisa lebih personal. <a href="https://maps.app.goo.gl/LE2YwZiM2exhqunh8" target="_blank" rel="noopener noreferrer" className="text-moon-bright border-b border-moon/40 hover:border-moon">Rekomendasi tempat</a></>,
                priceLabel: 'Rp 450K',
                offers: [
                    {
    ...
    return (
        <section id="services" className="relative isolate text-cream border-y border-white/[0.08]" style={{ background: 'linear-gradient(180deg, #111031 0%, #181140 55%, #1C1338 100%)' }}>
                            ...
                            <div className="max-w-[1600px] mx-auto px-6 md:px-10 lg:px-12">
                                ...
                                >
                                    <div className="flex items-center gap-x-3 gap-y-1.5 flex-wrap min-w-0">
                                        <h3 className="font-elegant font-medium text-cream text-[1.55rem] md:text-[2.2rem] leading-none tracking-tight transition-colors group-hover:text-moon-bright">
                                            {g.type}
                                        </h3>
                                        {g.seasonal && (
                                            <span className="text-[10px] uppercase tracking-[0.16em] font-semibold px-2.5 py-1 rounded-full bg-moon/20 text-moon-bright">
                                    ...
                                    <div className="flex items-center gap-4 md:gap-6 shrink-0">
                                        <span className="hidden sm:block text-sm md:text-[0.95rem] whitespace-nowrap">
                                            ...
                                            <span className="font-serif font-semibold text-moon-bright">{g.priceLabel}</span>
                                        </span>
                                        <span className={`grid place-items-center w-9 h-9 rounded-full border border-cream/25 transition-transform duration-300 ${open ? 'rotate-180 border-moon/60' : ''}`}>
                        ...
                        <ol className="mt-12 md:mt-16 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-5">
                            {steps.map((step, i) => (
                                <li key={i} className="animate-fade-up" style={{ animationDelay: `${i * 80}ms` }}>
                                    <span className="font-serif text-moon text-lg tabular-nums tracking-tight">

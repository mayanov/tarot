...
const Header: React.FC<HeaderProps> = ({ isIndonesian = false }) => {
  ...
  const navLinks: NavItem[] = [
    ...
    { name: isIndonesian ? 'Kenapa Saya?' : 'Values', id: 'why-choose' },
    ...
    { 
      ...
      children: isIndonesian ? [
        ...
        { name: 'Konsultasi Chat', id: 'service-chat' },
        { name: 'Panggilan Suara', id: 'service-call' },
        { name: 'Tatap Muka', id: 'service-meetup' }
      ] : undefined
    },
    { name: isIndonesian ? 'Ulasan' : 'Reviews', id: 'testimonials' },
    { name: isIndonesian ? 'Jejak' : 'Track Record', id: 'events' },
    { name: isIndonesian ? 'Tanya Jawab' : 'FAQ', id: 'faq' },
  ];
  ...
  return (
    ...
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-bg-dark/80 backdrop-blur-md py-4 shadow-lg border-b border-white/5' : 'bg-transparent py-6'
      }`}
    >
      ...
      {/* Mobile Nav Overlay */}
      <div 
        className={`lg:hidden fixed inset-0 bg-bg-dark/98 backdrop-blur-2xl z-[60] flex flex-col items-center justify-start pt-32 transition-all duration-500 ease-in-out ${
